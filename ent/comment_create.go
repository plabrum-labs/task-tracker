...
func (_c *CommentCreate) SetAt(v time.Time) *CommentCreate {
	_c.mutation.SetAt(v)
	return _c
}

// SetNillableAt sets the "at" field if the given value is not nil.
func (_c *CommentCreate) SetNillableAt(v *time.Time) *CommentCreate {
	if v != nil {
		_c.SetAt(*v)
	}
	return _c
...
func (_c *CommentCreate) check() error {
	if _, ok := _c.mutation.At(); !ok {
		return &ValidationError{Name: "at", err: errors.New(`ent: missing required field "Comment.at"`)}
	}
...
func (_c *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if value, ok := _c.mutation.At(); ok {
		_spec.SetField(comment.FieldAt, field.TypeTime, value)
		_node.At = value
	}
