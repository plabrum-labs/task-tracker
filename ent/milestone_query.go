...
func (_q *MilestoneQuery) QueryIssues() *IssueQuery {
...
func (_q *MilestoneQuery) WithIssues(opts ...func(*IssueQuery)) *MilestoneQuery {
	query := (&IssueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MilestoneQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Milestone, error) {
	...
	if query := _q.withIssues; query != nil {
		if err := _q.loadIssues(ctx, query, nodes,
			func(n *Milestone) { n.Edges.Issues = []*Issue{} },
			func(n *Milestone, e *Issue) { n.Edges.Issues = append(n.Edges.Issues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MilestoneQuery) loadIssues(ctx context.Context, query *IssueQuery, nodes []*Milestone, init func(*Milestone), assign func(*Milestone, *Issue)) error {
	...
	query.Where(predicate.Issue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(milestone.IssuesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.milestone_issues
		if fk == nil {
			return fmt.Errorf(`foreign-key "milestone_issues" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "milestone_issues" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
