(** The command line the tree in [lib/frontend/command.ml] is evaluated against, and nothing else.
...
let () =
  match Cmd.eval_value (Tt.Frontend.Command.main ()) with
  ...
  | Ok (`Ok (Error e)) ->
      prerr_endline (Tt.Platform.Error.to_string e);
