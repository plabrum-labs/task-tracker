...
// TestEnumsMatchSchema guards the plain casts in repository.go. The domain
// constants and the schema's enum values are two independent spellings of the
// same set, and a cast between them fails silently if one is renamed.
func TestEnumsMatchSchema(t *testing.T) {
	t.Parallel()

	for _, s := range []entissue.Status{
		entissue.StatusTodo, entissue.StatusDoing, entissue.StatusDone,
	} {
		if _, err := issue.ParseStatus(string(s)); err != nil {
			t.Errorf("schema status %q has no domain constant: %v", s, err)
		}
	}
	for _, s := range []issue.Status{
		issue.StatusTodo, issue.StatusDoing, issue.StatusDone,
	} {
		if err := entissue.StatusValidator(entissue.Status(s)); err != nil {
			t.Errorf("domain status %q is not a schema value: %v", s, err)
		}
	}

	for _, p := range []entissue.Priority{entissue.PriorityNormal, entissue.PriorityHi} {
		if _, err := issue.ParsePriority(string(p)); err != nil {
			t.Errorf("schema priority %q has no domain constant: %v", p, err)
		}
	}
	for _, p := range []issue.Priority{issue.PriorityNormal, issue.PriorityHi} {
		if err := entissue.PriorityValidator(entissue.Priority(p)); err != nil {
			t.Errorf("domain priority %q is not a schema value: %v", p, err)
		}
	}

	for _, k := range []issue.Kind{issue.KindDep, issue.KindSubtask} {
		if err := entref.KindValidator(entref.Kind(k)); err != nil {
			t.Errorf("domain kind %q is not a schema value: %v", k, err)
		}
	}
}
