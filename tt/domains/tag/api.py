...
every database call is wrapped in ``@with_transaction``. ``tag_list`` reads,
``tag_action`` writes — an object action passes the tag's id as its address, the
...
from tt.platform.actions import ActionDeps, ActionResponse, Field, Offer
