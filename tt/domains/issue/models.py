...
# A dependency edge between two issues of the same project: ``depends_on`` must be
# done before ``dependent`` can start. Issue-internal, so it lives here rather than
# in the tag module ``issue_tags`` sits in; a plain two-column link with no lifecycle
# of its own, cascaded away when either endpoint is hard-deleted.
issue_dependencies = Table(
    "issue_dependencies",
    BaseDBModel.metadata,
    Column("depends_on_id", ForeignKey("issues.id", ondelete="CASCADE"), primary_key=True),
    Column("dependent_id", ForeignKey("issues.id", ondelete="CASCADE"), primary_key=True),
)
...
class Issue(BaseDBModel):
    ...
    # The dependency graph, self-referential over ``issue_dependencies`` and split by
    # which end of the edge this issue sits on. ``depends_on`` is the issues that must
    # finish before this one (this row is ``dependent_id``); ``dependents`` is the
    # issues that wait on this one (this row is ``depends_on_id``). Same-project, no
    # self-edge and no cycles are the actions' rules, not the mapping's. The two map
    # the same table from opposite ends, so exactly one may own the writes: an edge is
    # added and removed through ``depends_on``, and ``dependents`` is the read-only mirror.
    depends_on: Mapped[list[Issue]] = relationship(
        secondary=issue_dependencies,
        primaryjoin=lambda: Issue.id == issue_dependencies.c.dependent_id,
        secondaryjoin=lambda: Issue.id == issue_dependencies.c.depends_on_id,
        lazy="raise",
    )
    dependents: Mapped[list[Issue]] = relationship(
        secondary=issue_dependencies,
        primaryjoin=lambda: Issue.id == issue_dependencies.c.depends_on_id,
        secondaryjoin=lambda: Issue.id == issue_dependencies.c.dependent_id,
        viewonly=True,
        lazy="raise",
    )
