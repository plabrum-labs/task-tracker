...
def save(prefs: Prefs) -> None:
    ...
    of the existing document is loaded and preserved — a hand-edited table the app
    does not own survives the TUI persisting a theme, which would otherwise erase
    every table this rebuild did not list."""
    path = config_path()
...
# --- the [database] table --------------------------------------------------
#
# The Postgres a device connects to. Hand-edited, not written here — a URL with a
# password does not belong in a file the TUI rewrites — so there is no save
# counterpart, and ``save``'s preservation of unowned tables is what keeps a theme
# write from erasing it.


def load_database_url() -> str | None:
    """The ``[database] url`` a device connects to, or ``None`` when the file names
    none. Total like ``load``: a missing or malformed table yields ``None`` so the
    caller falls back to its own default rather than crashing on a hand-edited
    file."""
    data = _raw_document()
    database = data.get("database")
    database = database if isinstance(database, dict) else {}
    return _text(database, "url")
